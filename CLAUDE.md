...
- UX pass (2026-07): new `src/components/ui/MoneyInput.jsx` — comma-formatted, cursor-preserving `type="text" inputMode="numeric"` currency input — replaces every `type="number"` money field app-wide (`AmountFrequencyInput`, bonus, fixed-expense amounts, additional-loan amounts, `ScenarioPanel` overrides, `PayslipReviewCard` net pay); `AmountFrequencyInput` `cycleLabel` fixed to handle `weekly` (was falling through to "month"); blank quick-add/custom expense and loan rows now get an amber border + inline warning plus a count near Next; new `src/components/layout/ScrollToTop.jsx` mounted once in `App.jsx` resets scroll on every route change; `ScenarioPanel` auto-activates (`scenario.active = true`) as soon as a value is typed into any override input, and its toggle now has a real label instead of `label=""`; `BucketBreakdown`'s `SectionHeader` gained an `editRoute` prop — Income/Regular/Fixed/Savings cards now deep-link to their wizard step instead of only the header "Edit" button (which still resets to Income); `ExpenseCalendar` now merges enabled `householdBills` into the schedule (previously `fixedExpenses` only); new `SavingsGoalProgress` shown inside the Savings card in `BucketBreakdown` — surfaces `savingsGoalAmount`, which `calculateBudget` already computed but nothing displayed; new `src/components/dashboard/ToolSuggestion.jsx` — dismissible cross-tool suggestion card (FIRE Calculator if savings rate meets target, else Salary Sacrifice/Super Compare) using a pure `selectSuggestion(budget, profile)` selector so `Dashboard` can `key={}` the component and force a remount when the suggestion changes (needed because `useStorage`'s dismissed-flag read only happens once per mount); new `src/components/wizard/RunningTotalBar.jsx` — sticky bar under `StepNavBar`, shown only on `/wizard/housing`, `/wizard/groceries`, `/wizard/fixed`, showing income vs. expenses-so-far vs. remaining per cycle
...
Brackets: 0% / 16% / 30% / 37% / 45% at thresholds 18,200 / 45,000 / 135,000 / 190,000. LITO max $700, phases out $37,500–$66,667. Medicare levy 2%. All values live in `src/utils/constants.js`. Note: the first bracket drops to 15% from the 2026–27 income year.
...
Design system: Plus Jakarta Sans, slate palette (slate-900 bg, slate-800 card, slate-700 border — **unmigrated**, still the old KashVector dark palette by coincidence, since Tailwind's default `slate-900` literally equals `#0f172a`), `primary`/`primary-dark`/`primary-contrast`/`accent` Tailwind colors (buttons, focus rings, active states — **migrated to Ink & Amber 2026-07-22**, see below). Semantic colours: emerald = income/savings/positive, rose = fail/over-budget, amber = warning (unmigrated, stays close to `--kv-warn` in both modes so no change was needed). All interactive elements `min-h-[44px]` (Capacitor tap target). Dark mode via `dark:` Tailwind variants throughout. See `C:\Projects\Rules\kashvector-design.md` for full token reference.

**Ink & Amber rebrand (2026-07-22) — accent/warn colors only, base slate ramp deliberately left unmigrated.** Full rationale in `Kashvector.md`'s "Ink & Amber rebrand — rollout status" (Phase 5) in `C:\Projects\StockAnalysis` — summary:
- `tailwind.config.js`'s `primary`/`accent` colors (previously unused hardcoded hex — nothing in the codebase referenced them by name) now reference CSS custom properties declared in `src/index.css` (`--kv-bg`/`--kv-accent`/`--kv-accent-h`/`--kv-warn`, `:root` = dark "Ink & Amber", `html:not(.dark)` = light "Paper & Ink"). Use `bg-primary`/`text-primary`/`ring-primary`/`text-primary-contrast` (for on-accent text) instead of literal `sky-*` classes — no separate `dark:` variant needed, the CSS var inverts automatically.
- Tailwind can't apply opacity modifiers to `var()`-based arbitrary colors (a build-time RGB-channel limitation). For tinted/translucent accent backgrounds or halo rings, use `bg-[color-mix(in_srgb,var(--kv-accent)_8%,transparent)]`-style arbitrary values instead of `bg-primary/10`.
- `amber-*` usage was audited per-instance and almost entirely left unchanged (categorical/warning colors, not accent-tied) — only literal `sky-*` accent usage was migrated. Don't assume every amber/sky sighting needs the same treatment; check whether it's tied to `primary` (accent, must invert) or is a standalone status/category color (can stay).
- Deploy still requires `MSYS_NO_PATHCONV=1 VITE_BASE_PATH='/budget/' npm run build` + copying `dist/` to `StockAnalysis/www/budget/` — this rebrand didn't change the deploy process, see below.
